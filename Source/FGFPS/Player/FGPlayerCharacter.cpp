...
void AFGPlayerCharacter::Tick(float DeltaSeconds)
{
	...
	if (bIsCarryingKey == true)
	{
		OnObjectivePickup();

		if (MyExtractionZone != nullptr)
		{
		FVector Direction = GetActorLocation() - MyExtractionZone->GetActorLocation();
		Direction.Normalize();

		FVector Forward = GetActorForwardVector();

		//UImage* Image = Cast<UImage>(GetWidgetFromName("WBP_PointExtractZone"));

		float DirAtan = FMath::Atan2(Direction.X, Direction.Y);
		float FwdAtan = FMath::Atan2(Forward.X, Forward.Y);

		FinalAngle = FwdAtan - DirAtan;
		FinalAngle = FMath::RadiansToDegrees(FinalAngle);

		/*if (Image)
		{*/
			UE_LOG(LogTemp, Warning, TEXT("Angle: %f"), FinalAngle);
		}
	}
